...
export default function ListDysfonctionnements({ pkImmeuble }: ListDysfonctionnementsProps) {
  ...
  const handleExportDysfonctionnements = useCallback(async () => {
    await exportDysfonctionnements(pkImmeuble);
  }, [exportDysfonctionnements, pkImmeuble]);

  // Use the reusable export hook
  const { handleExport, isExporting, error: exportError, clearError: clearExportError } = useExport(handleExportDysfonctionnements);
      ...
      <div className="mb-6 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={isExporting || dysfonctionnements.length === 0}
