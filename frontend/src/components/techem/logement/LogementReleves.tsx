...
export default function LogementReleves({ 
  ...
  const relevesData = useMemo(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('Logement data structure:', logement);
      console.log('LogementRepart:', (logement as Record<string, unknown>)?.LogementRepart);
      console.log('Has LogementRepart:', 'LogementRepart' in (logement || {}));
    }
    
    // For logement, we might need to get data from the immeuble or from logement-specific structures
    // Assuming similar structure but adapted for logement
    // Note: For a single logement, we might not have ImmeubleEF/EC/Repart/CET structures
    // These might need to come from the parent immeuble or be calculated differently
    // For now, we'll try to access them if they exist, otherwise return empty data
    
    // Try to get data from logement or its parent immeuble
    ...
    // Try logement first, then immeuble
    const logementRepart = (logement && typeof logement === 'object' && 'LogementRepart' in logement)
      ? (logement as { LogementRepart?: Record<string, unknown> }).LogementRepart
      : null;
    ...
    // Use logement repart if available, otherwise fallback to immeuble repart
    const repartData = logementRepart || immeubleRepart;

    // Calculate percentage for Répartiteur
    const nbCompteursRepartRelevesRaw = repartData?.NbCompteursReleves ?? repartData?.nbCompteursReleves ?? 0;
    const nbCompteursRepartAReleverRaw = repartData?.NbCompteursARelever ?? repartData?.nbCompteursARelever ?? 0;
    ...
    const topConsosRepart = (repartData && typeof repartData === 'object' && 'TopConsos' in repartData)
      ? (repartData as { TopConsos?: Record<string, unknown> }).TopConsos
      : null;
    ...
    // Try logement first, then immeuble
    const logementCET = (logement && typeof logement === 'object' && 'LogementCET' in logement)
      ? (logement as { LogementCET?: Record<string, unknown> }).LogementCET
      : null;
    ...
    // Use logement CET if available, otherwise fallback to immeuble CET
    const cetData = logementCET || immeubleCET;

    // Calculate percentage for Compteur d'énergie
    const nbCompteursCETRelevesRaw = cetData?.NbCompteursReleves ?? cetData?.nbCompteursReleves ?? 0;
    const nbCompteursCETAReleverRaw = cetData?.NbCompteursARelever ?? cetData?.nbCompteursARelever ?? 0;
    ...
    const topConsosCET = (cetData && typeof cetData === 'object' && 'TopConsos' in cetData)
      ? (cetData as { TopConsos?: Record<string, unknown> }).TopConsos
      : null;
